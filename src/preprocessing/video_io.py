...
"""
Video preprocessing utilities for the SmartCampus video-to-text project.

This module handles:
- reading raw videos
- sampling frames
- filtering dark and lazy frames
- DNN-based face anonymization with strong blur
- resizing frames to model input size
- saving processed frames and metadata
"""
...
def prepare_video(
    ...
    change_threshold = float(video_cfg.min_change_threshold)
    ...
    while True:
        ...
        if _is_dark_frame(small_gray, brightness_threshold=dark_threshold):
            num_dark_skipped += 1
            frame_idx += 1
            continue
        ...
        resized = _resize_frame(frame, target_size=model_input_size)
...
def _ensure_prepared_dirs(
    ...
    prepared_root = prepared_base / video_id
...
def _resize_frame(frame: np.ndarray, target_size: Tuple[int, int]) -> np.ndarray:
    h, w = target_size
